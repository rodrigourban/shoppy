...
CATEGORY_1 = {"name": "beverages", "slug": "beverages"}
...
MESSAGE_1 = {"content": "Hello, I want to ask a question regarding this tea"}

MESSAGE_2 = {"content": "Sure, I'm glad I can help you. What's your question?"}
...
class CustomerCommunicationTests(TestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        cls.product = Product.objects.create(
            **PRODUCT_1, category=cls.category, created_by=cls.superuser
        )
    ...
    def test_communications_list_view_403(self):
        ...
        self.assertRedirects(
            response, f"{reverse('account_login')}?next=/communication/"
        )

    def test_communications_list_view_no_conversations(self):
        self.client.login(
            email=USER_CUSTOMER["email"], password=USER_CUSTOMER["password"]
        )
        response = self.client.get(self.url_list)
    ...
    def test_communications_create_view_404(self):
        ...
        response = self.client.get(
            reverse("communication:create_detail", args=[1231312])
        )
    ...
    def test_communications_create_view(self):
        ...
        response = self.client.get(
            reverse("communication:create", args=[self.product.pk])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_communications_create(self):
        ...
        response = self.client.post(
            reverse("communication:create", args=[self.product.pk]), MESSAGE_1
        )
    ...
    def test_communications_list_view(self):
        conversation = Conversation.objects.create(product=self.product)
    ...
    def test_communications_list_view_unread_notification(self):
        ...
        self.assertContains(
            response, f'{new_message_2.created_at.strftime("%d/%m/%Y %H:%M")}'
        )
    ...
    def test_communications_list_view_mark_as_read(self):
        ...
        response = self.client.get(
            reverse("communication:create_detail", args=[conversation.pk])
        )  # open unread message
        response = self.client.get(self.url_list)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Messages")
...
class AdminCommunicationTests(TestCase):
    ...
    def test_communications_list_view_no_conversations(self):
        self.client.login(
            username=USER_ADMIN["username"], password=USER_ADMIN["password"]
        )
        response = self.client.get(self.url_list)
        self.assertEqual(response.status_code, 200)
