...
class CustomerCommunicationTests(TestCase):
  ...
  def test_communications_create_view_403(self):
    response = self.client.get(reverse('communication:create', args=[1]))
    self.assertEqual(response.status_code, 302)
    self.assertRedirects(
      response,
      f"{reverse('account_login')}?next=/communication/create/1/"
    )
  
  def test_communications_create_view_404(self):
    self.client.login(email=USER_CUSTOMER['email'], password=USER_CUSTOMER['password'])
    response = self.client.get(reverse('communication:create', args=[1231312]))
  ...
  def test_communications_create_view(self):
    ...
    response = self.client.get(reverse('communication:create', args=[1]))
    self.assertEqual(response.status_code, 200)
    self.assertContains(response, 'Ask us about this product!')
    self.assertTemplateUsed(response, 'communication/create.html')

  def test_communications_create(self):
    self.client.login(email=USER_CUSTOMER['email'], password=USER_CUSTOMER['password'])
    response = self.client.post(reverse('communication:create', args=[1]), MESSAGE_1)
    self.assertEqual(response.status_code, 200)
    conversation = Conversation.objects.first()
    self.assertEqual(Conversation.objects.count(), 1)
    self.assertEqual(Message.objects.count(), 1)
    self.assertEqual(conversation.members_set.count, 3) # user and all superusers

  def test_communications_list_view(self):
    response = self.response.client.get(self.url_list)
    self.assertEqual(response.status_code, 200)
    self.assertContains(response, 'Messages')
    self.assertContains(response, PRODUCT_1['name'])
    self.assertContains(response, MESSAGE_1['content'][:10])
  ...
  def test_communications_list_view_unread_notification(self):
    ...
    self.assertContains(response, 'Messages')
    self.assertContains(response, PRODUCT_1['name'])
    self.assertContains(response, MESSAGE_2['content'][:10])
    self.assertContains(response, 'Sent on')
    self.assertContains(response, 'New message')
    self.assertContains(response, new_message_2.created_at)
  ...
  def test_communications_list_view_mark_as_read(self):
    ...
    response = self.client.get(reverse('communication:detail', args=[self.conversation.pk])) # open unread message
    response = self.response.client.get(self.url_list)
    self.assertEqual(response.status_code, 200)
    self.assertContains(response, 'Messages')
    self.assertContains(response, PRODUCT_1['name'])
  ...
  def test_communications_detail_view_403(self):
    self.client.logout()
    response = self.client.get(reverse('communication:detail', args=[self.conversation.pk]))
    self.assertEqual(response.status_code, 302)
    self.assertRedirects(
      response,
      f"{reverse('account_login')}?next=/communication/detail/"
    )
  
  def test_communications_detail_view_404(self):
    self.client.login(email=USER_CUSTOMER['email'], password=USER_CUSTOMER['password'])
    response = self.client.get(reverse('communication:detail', args=[1231312]))
    self.assertEqual(response.status_code, 404)
